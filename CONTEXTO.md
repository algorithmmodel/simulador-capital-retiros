...
- **Los retiros se cargan en dólares de hoy.** Desde el 13 de septiembre de 2026. El monto del retiro y el de cada ajuste se interpretan como poder de compra actual, y en cada semestre sale `monto * acum[i]`: lo que hoy son 90.000 llevado por la inflación hasta ese semestre. Antes el monto se tomaba en dólares del momento del primer retiro y la indexación arrancaba ahí, lo cual era una incoherencia —se suponía inflación para los años posteriores al primer retiro pero no para los anteriores— y además engañaba: con el primer retiro a tres años, "90.000" compraba lo que hoy compran 82.364. La interfaz lo dice con la expresión "poder de compra actual" (ver el punto siguiente) y muestra, en el resultado y en el pie de cada modelo, cuánto sale efectivamente de la cuenta en el primer retiro. Los retiros sostenibles también quedan en poder de compra actual, en la misma moneda que el retiro planteado, así que la comparación es directa. La tabla semestre a semestre y el total retirado siguen en nominal, porque muestran los dólares que efectivamente salen de la cuenta. **No volver a indexar desde el primer retiro.**
- **"Poder de compra actual", no "de hoy".** Implementado el 13 de septiembre de 2026 a pedido de Omar. Se descartó "actual" solo porque en finanzas "valor actual" es el monto descontado a una tasa de interés, y acá solo se descuenta inflación; se descartó "real" por menos claro. La frase es larga y **no se repite en cada rótulo** —probado sobre un dibujo: parte los rótulos en dos o tres renglones y desalinea los números—; va una sola vez por lugar: la ayuda del campo de retiro ("En poder de compra actual."), la de los ajustes ("También en poder de compra actual."), una franja arriba de las tarjetas de modelos y de las de capital final, y la leyenda chica debajo de cada capital final. Los rótulos quedan cortos: "Retiro semestral (USD)", "Nuevo retiro USD", "Retiro sostenible", "Su retiro", "Retiro máximo".
- **Objetivo de capital final en dólares.** Implementado el 13 de septiembre de 2026. El deslizante sigue siendo el valor por defecto; debajo hay un campo "Objetivo en USD (opcional)". **Si el campo tiene valor, manda ese; si está vacío, manda el deslizante** (`objetivoVigente()`). Existe porque el deslizante va de 5 en 5 y un objetivo concreto no se podía fijar: $100.000 sobre $3.530.000 es 2,8%. La lectura debajo del deslizante dice cuál de los dos está vigente, y el porcentaje que se muestra en resultados, resumen, PDF y comparación es el efectivo (`pctTxt`, con un decimal cuando viene del monto). Verificado con el escenario de la captura: con $100.000 los máximos pasan a 192.848 / 91.672 / 91.650 y el plan aprueba los dos modelos (97,5% y 95,4%); al vaciar el campo vuelve el 20%.
- **Pie de las tarjetas de los modelos.** Implementado el 13 de septiembre de 2026, aprobado sobre un dibujo. Reemplaza al "Capital requerido para retirar…". Dos renglones: *"En Año 3 / S1 sale: $93,552"* —el retiro sostenible de ese modelo llevado por la inflación cargada hasta el primer retiro, en azul— y *"Faltan $126,414 de capital"* o *"Sobran…"*, grande, en rojo o verde. El cálculo del sobra/falta sigue siendo `invertido − invertido × retiro / sostenible`, válido porque el motor es lineal en los montos. Las celdas de arriba son **una grilla de 2×2** —rótulos en la primera fila, números en la segunda— para que los números arranquen a la misma altura aunque un rótulo o un número ("73 de 79 (92.4%)") ocupe dos renglones. Antes cada celda era una columna propia, con `.modo .cifras div` sin `>`, que además dibujaba dos rayas por celda.
- **Capital al final: cada retiro con su capital final.** Implementado el 13 de septiembre de 2026, aprobado sobre un dibujo. Reemplaza las tarjetas "Capital final nominal" y "Capital final real", que se eliminaron. Resuelve una confusión real: en pantalla convivían el retiro planteado y el máximo, y el capital final grande no decía con cuál se calculó; Omar leyó "$187.899 de retiro sostenible" junto a "$12.693.394 de capital final" como un mismo escenario, cuando con el máximo el capital final es el objetivo. Una tarjeta por modelo con dos pares: *Su retiro → Capital final* y *Retiro máximo → Capital final* (en azul). Debajo de cada capital final, su valor en poder de compra actual en naranja. En el CAGR fijo es exacto; en histórico y Monte Carlo el número grande es la **mediana** y abajo va el **peor X%** (X = 100 − nivel exigido), en verde si alcanza el objetivo y en rojo si no. Ese peor caso **no es un percentil interpolado**: es el caso ordenado que decide el éxito (`peorNivel`: si al nivel exigido pueden fallar como mucho k casos, se muestra el k+1). Con el percentil interpolado el retiro máximo de Monte Carlo mostraba 705.045 en rojo contra un objetivo de 706.000, aunque cumplía por definición. Con el criterio correcto, el peor caso del retiro máximo cae justo sobre el objetivo, que es lo que significa "máximo". Números completos, sin abreviar. Para esto Monte Carlo guarda los dos capitales finales de cada corrida (números, no la simulación entera) y se corre una vez más cada modelo con su retiro máximo (`finalesCon`). Referencia verificada con el escenario de la captura (capital 3.500.000, tres aportes de 10.000, CAGR 10%, IPC real, retiro 90.000 desde Año 3/S1, objetivo 20%, nivel 95%): CAGR fijo 90.000 → 12.693.394 (7.028.025) y 187.899 → 706.000 (390.895); histórico 90.000 → 11.669.557 (6.601.563, peor 5% 468.301, rojo) y 86.888 → 12.047.889 (6.721.245, peor 5% 706.001, verde); Monte Carlo 90.000 → 13.187.905 (7.045.720, peor 5% 299.678, rojo) y 84.204 → 13.917.903 (7.530.368, peor 5% 706.000, verde).
- **Aportes y cambios de retiro no bloquean nunca el cálculo.** Desde el 13 de septiembre de 2026, a pedido de Omar. Antes, un ajuste de retiro anterior al primer retiro —justo lo que creaba "Agregar ajuste", en Año 2 con $0— se mostraba como error y deshabilitaba el botón, y el motivo quedaba arriba de todo, fuera de la vista. Ahora: las filas en **$0 se ignoran** (en las dos secciones), un cambio de retiro anterior al primer retiro **se ignora sin aviso** (el motor ya lo ignoraba), y una fila con monto fuera del horizonte deja una **nota naranja que no bloquea**. Un aporte puede ir en cualquier semestre, antes o después de empezar a retirar, y siempre se sumó así. Consecuencia buscada: un cambio de retiro en $0 no sirve para "dejar de retirar"; para eso hay que cargar un monto chico. Por defecto la pantalla trae **una sola fila de aporte, Año 2 en $0**, para completar; los tres aportes de 10.000 dejaron de ser el valor por defecto. La sección "Ajustes puntuales del retiro" se llama ahora **"Cambio de retiros"** y su botón "Agregar cambio", porque "ajustes" se confundía con "aportes".
...
**Configuración de referencia.** Desde el 13 de septiembre de 2026 **ya no coincide con los valores por defecto de la app**, que traen una sola fila de aporte en $0: para correr el test hay que cargar a mano los tres aportes de 10.000. Configuración: capital 100.000, 20 años, CAGR 9%, inflación 3%, aportes de 10.000 en años 2/3/4 (semestre 1), retiro 2.400 semestral desde año 5 semestre 1, objetivo 100% nominal, orden A, inflación constante.

| | Configuración de referencia | La misma ×5 |
|---|---|---|
...
Con la configuración de referencia pero el objetivo en **0%**, el sostenible con CAGR fijo tiene que dar **7.574** (37.870 en la escala ×5) y no agotar el capital. Ese caso es el que detecta si alguien volvió a sacar el chequeo de agotamiento de la búsqueda (ver sección 6).

Con la configuración de referencia pero el selector de inflación en **IPC real de cada año**: éxito histórico 78 de 79, éxito Monte Carlo 91,9%, sostenible histórico al 90% **3.119** (15.594 ×5), sostenible Monte Carlo al 90% **2.834** (14.171 ×5). El CAGR fijo no se mueve, porque ese modelo siempre usa la inflación constante.
